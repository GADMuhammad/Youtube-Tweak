...
- **Quick Search:** Search YouTube straight from the popup without opening the site first.
  ...
  - RTL-aware: each suggestion's text direction follows its own language.
...
- Fixed videos/Shorts sometimes auto-loading an extra batch as soon as a feed page opened (e.g. the Shorts subscriptions feed), caused by the same load-more race previously fixed for comments; the early CSS-based block is now applied to every feed, not just comments.
